...
This is a minor variation from the previous select. Here we can specify a subset of the _attributes_ to be retrieved.
...
try:
    request = (
        bios.isql()
        .select("countryCode", "reportedCases")
...
try {
    final var statement = Bios.isql().select("countryCode", "reportedCases")
