...
export const LOAD_AUTH = 'isima/login/LOAD_AUTH';
export const LOAD_AUTH_SUCCESS = 'isima/login/LOAD_AUTH_SUCCESS';
export const LOAD_AUTH_ERROR = 'isima/login/LOAD_AUTH_ERROR';
 ...
 */
export function authLoaded(auth) {
  return {
...
function* workerGetUserAuthSaga({ credential }) {
  try {
    const auth = yield call(validateUser.bind(this, credential));
    yield put(authLoaded(auth));
  } catch (error) {
    yield put(authLoadingError(error));
  }
}

export function* watchGetUserAuthSaga() {
  yield takeLatest(LOAD_AUTH, workerGetUserAuthSaga);
}
