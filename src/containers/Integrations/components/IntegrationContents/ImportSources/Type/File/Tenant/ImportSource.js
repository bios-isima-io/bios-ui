...
const { setFileIntegration } = fileIntegrationActions;

function ImportSource({ setFileIntegration, fileLocation, pollingInterval }) {
  const [activeRightPanelSection, setActiveRightPanelSection] = useState('');

  const setFileData = (key, value) => {
    setFileIntegration({ [key]: value });
  };
...
const mapStateToProps = (state) => {
  const { fileLocation, pollingInterval } = state?.integration?.file;
  return {
    fileLocation,
    pollingInterval,
  };
