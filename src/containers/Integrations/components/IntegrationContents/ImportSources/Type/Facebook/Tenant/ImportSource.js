...
const { setFacebookIntegration } = facebookIntegrationActions;
...
function ImportSource({
  ...
  const setFacebookIntegrationData = (key, value) => {
    setFacebookIntegration({ [key]: value });
  };
        ...
        component: (
          <div className={css(styles.collapsableDiv)}>
            <SourceProperties
              ...
              setFacebookIntegrationData={setFacebookIntegrationData}
            />
          </div>
        ),
...
const mapStateToProps = (state) => {
  const { clientId, clientSecret, accessToken, endpoint, pollingInterval } =
    state?.integration?.facebook;
  return {
    clientId,
    clientSecret,
    accessToken,
