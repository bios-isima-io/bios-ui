...
const { setRdbmsIntegration } = rdbmsIntegrationActions;
...
function ImportSource({
  ...
  const setRdbmsData = useCallback(
    (key, value) => {
      setRdbmsIntegration({ [key]: value });
    },
    [setRdbmsIntegration],
  );
  ...
  const setSSLMode = useCallback(
    (key, value) => {
      setRdbmsIntegration({ [key]: { mode: value } });
    },
    [setRdbmsIntegration],
  );
  ...
  useEffect(() => {
    return () => {
      setRdbmsData('srcUser', '');
      setRdbmsData('srcPassword', '');
      setRdbmsData('type', '');
    };
  }, [setRdbmsData]);
