...
import PayloadVerification from 'containers/Integrations/components/BiosDestination/PayloadVerification';
...
function ImportSource({ srcWebhookPath, setWebhookIntegration }) {
  const [activeRightPanelSection, setActiveRightPanelSection] = useState(null);

  const setWebhookData = (key, value) => {
    setWebhookIntegration({ [key]: value });
  };
...
const mapStateToProps = (state) => {
  const { srcWebhookPath } = state?.integration?.webhook;
  return {
    srcWebhookPath,
  };
};
...
ImportSource.propTypes = {
  setWebhookIntegration: PropTypes.func.isRequired,
  srcWebhookPath: PropTypes.string.isRequired,
};
