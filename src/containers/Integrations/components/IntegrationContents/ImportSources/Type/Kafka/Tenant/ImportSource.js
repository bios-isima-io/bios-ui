...
function ImportSource({
  ...
  useEffect(() => {
    return () => {
      setKafkaData('srcUser', '');
      setKafkaData('srcPassword', '');
    };
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, []);

  const setKafkaData = (key, value) => {
    setKafkaIntegration({ [key]: value });
  };
  ...
  const setKafkaBootstrapServersData = (key, index, value) => {
    ...
    setKafkaIntegration({ [key]: bootstrapServersCopy });
  };
...
const mapStateToProps = (state) => {
  const { srcUser, srcPassword, apiVersion, bootstrapServers } =
    state?.integration?.kafka;
  return {
    srcUser,
    srcPassword,
    apiVersion,
