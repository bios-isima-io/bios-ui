...
function SourceProperties({
  bootstrapServers,
  setKafkaBootstrapServersData,
  kafkaBootstrapRemoveServer,
  kafkaBootstrapAddServer,
  apiVersion,
  srcUser,
  srcPassword,
  setKafkaData,
  ...
  return (
    <div>
      <div className={css(styles.rPanelSubSectionRow)}>
        <div className={css(styles.bootStrapServerContainer)}>
          Bootstrap Servers
        </div>
        <div>
                ...
                {disableFieldEdit ? (
                  ...
                  <div className={css(styles.inputWrapper)}>
                    <Input
                      ...
                      hideSuffix={true}
                      placeholder="Eg. 34.93.18.170:9092"
                      onChange={(event) => {
                        setErrors(removeObjectKey(errors, 'bootstrapServers'));
                        setKafkaBootstrapServersData(
                          'bootstrapServers',
                          index,
                          event.target.value,
                    ...
                    <div className={css(styles.inputDeleteIcon)}>
                      <i
                        ...
                        onClick={() => {
                          ...
                          kafkaBootstrapRemoveServer('bootstrapServers', index);
                        }}
                      />
                    </div>
                  </div>
          ...
          {!disableFieldEdit && (
            <Button
              ...
              size="small"
              onClick={() => {
                kafkaBootstrapAddServer('bootstrapServers');
              }}
            >
              Add Server
            </Button>
      ...
      <div className={css(styles.rPanelSubSectionRow)}>
        <div className={css(styles.rPanelSubSectionCol1)}>API Version</div>
        <div>
          {disableFieldEdit ? (
            <RenderUneditableText text={apiVersion} />
          ) : (
            <Input
              error={errors.apiVersion}
              hideSuffix={true}
              placeholder="Eg. 0.10.2"
              onChange={(event) => {
                setErrors(removeObjectKey(errors, 'apiVersion'));
                setKafkaData('apiVersion', event.target.value);
              }}
              value={apiVersion}
              disabled={disableFieldEdit}
            />
          ...
          {disableFieldEdit ? (
            ...
            <Input
              ...
              onChange={(event) => {
                ...
                setKafkaData('srcUser', event.target.value);
              }}
              disabled={disableFieldEdit}
            />
          )}
          ...
          {disableFieldEdit ? (
            ...
            <InputPassword
              ...
              onChange={(event) => {
                ...
                setKafkaData('srcPassword', event.target.value);
              }}
              disabled={disableFieldEdit}
            />
          )}
...
SourceProperties.propTypes = {
  bootstrapServers: PropTypes.array,
  setKafkaBootstrapServersData: PropTypes.func,
  kafkaBootstrapRemoveServer: PropTypes.func,
  kafkaBootstrapAddServer: PropTypes.func,
  apiVersion: PropTypes.string,
  srcUser: PropTypes.string,
  srcPassword: PropTypes.string,
  setKafkaData: PropTypes.func,
