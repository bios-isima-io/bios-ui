...
import ImportDataProcessors from './ImportDataProcessors';
import ImportSources from './ImportSources/ImportSources';
...
function IntegrationContents({
  ...
  const tabsConfig = {
    defaultTab: TAB_SOURCE,
    tabs: [
      {
        label: isMobile ? 'Import\nSources' : 'Import Sources',
        key: TAB_SOURCE,
        content: () => {
          return <ImportSources history={history} />;
        },
      },
      {
        label: isMobile ? 'User-defined\nFunctions' : 'User-defined Functions',
        key: TAB_PROCESS,
        content: () => {
          return <ImportDataProcessors history={history} />;
        },
      },
      {
        ...
        key: TAB_DESTINATION,
        content: () => {
          return <ExportDestinations history={history} />;
        },
      },
    ],
  };
  ...
  const onTabChange = (key) => {
    ...
    if (rightPanelActive) {
      const selected = trySelectingContent(key);
      if (!selected) {
  ...
  const getContent = () => {
    return (
      <div className={css(styles.wrapper)}>
        <Tabs tabsConfig={tabsConfig} onTabChange={onTabChange} />
      </div>
    );
  };

  return <div className={css(styles.integrationList)}>{getContent()}</div>;
}

IntegrationContents.propTypes = {
...
const mapStateToProps = (state) => {
  const { importSourcesCopy, rightPanelActive, importDataProcessorsCopy } =
