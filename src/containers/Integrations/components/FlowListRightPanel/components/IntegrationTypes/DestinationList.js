...
function DestinationList({
  ...
  const { importDestinationId } = destinationDataSpec;
  const destinationMenu = (
    <Menu
      onClick={(val) => {
        setDestinationDataSpec({ importDestinationId: val?.key });
      }}
    >
      {importDestinations?.map((dest) => (
        <Menu.Item key={dest.importDestinationId}>
          {dest.importDestinationName}
        </Menu.Item>
      ))}
    </Menu>
  );
  ...
  return (
    <div className={css(styles.rPanelSubSectionRow)}>
      <div className={css(styles.rPanelSubSectionCol1)}>Destination</div>
      <div>
            ...
            <div>
              {
                importDestinations?.find(
                  (dest) => dest.importDestinationId === importDestinationId,
...
const mapStateToProps = (state) => {
  const { importDestinations } = state?.integration?.integrationConfig;
...
DestinationList.propTypes = {
  ...
  destinationDataSpec: PropTypes.object,
};
