...
import { updateSourceChange } from 'containers/Integrations/components/FlowListRightPanel/utils';
import styles from 'containers/Integrations/styles';
import webhookStyles from './styles';
...
function ImportFlow({
  ...
  const { importSourceId, payloadType, webhookSubPath } = sourceDataSpec;
  const webhookPath = importSources?.filter(
    (is) => importSourceId === is.importSourceId,
  )?.[0]?.webhookPath;
  ...
  const webhookURL = `${config.backend}/integration/${tenant}${
    webhookPath ? webhookPath : ''
  }${webhookSubPath ? webhookSubPath : ''}`;
  return (
    <div className={css(styles.rightPanelSectionContainer)}>
      <div className={css(styles.rPanelSubSectionRow)}>
        <div className={css(styles.rPanelSubSectionCol1)}>Webhook Sub Path</div>
        <div>
          <Input
            value={webhookSubPath === undefined ? '' : webhookSubPath}
            hideSuffix={true}
            placeholder=""
            onChange={(event) =>
              updateSourceData('webhookSubPath', event.target.value)
            }
          ></Input>
        </div>
      </div>
      ...
      <div className={css(webhookStyles.webhookUrlContent)}>
        Note: Setup the source system to <strong>POST</strong> data to
        <div className={css(webhookStyles.webhookUrl)}>
          <Tooltip title={webhookURL}>{webhookURL}</Tooltip>
        </div>
      </div>
    </div>
  );
...
const mapStateToProps = (state) => {
  ...
  const { importSources } = state.integration.integrationConfig;
