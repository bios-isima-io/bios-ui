...
function ImportFlow({
  ...
  const { payloadType, headerPresent, fileNamePrefix, s3Bucket } =
    sourceDataSpec;
  ...
  return (
    <div className={css(styles.rightPanelSectionContainer)}>
      <div className={css(styles.rPanelSubSectionRow)}>
        <div className={css(styles.rPanelSubSectionCol1)}>S3 Bucket</div>
        <div>
          <Input
            value={s3Bucket === undefined ? '' : s3Bucket}
            hideSuffix={true}
            placeholder=""
            onChange={(event) =>
              updateSourceData('s3Bucket', event.target.value)
            }
          ></Input>
        </div>
      </div>
