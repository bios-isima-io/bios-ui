...
import { WORD_CLOUD_TABLE_CONFIG } from './constant';
import styles from './styles';
import { useMemo } from 'react';

const columns = WORD_CLOUD_TABLE_CONFIG.map(({ title, dataIndex, width }) => {
  return {
    title,
    dataIndex,
    width,
...
const WordCloudTable = ({
  data = [],
  isStringType,
  distinctCountAbove256Limit,
}) => {
  let tableBodyData = [];
  let currentColumns = isStringType ? [...columns] : columns.slice(0, 3);
  ...
  if (data) {
    let tableData = [];
    data?.sample?.forEach((_, index) => {
      const sample = data?.sample?.[index];
      const sampleCount = Humanize.compactInteger(data.sampleCount?.[index], 2);
      const sampleLength = Humanize.compactInteger(
        data.sampleLength?.[index],
        2,
      );
