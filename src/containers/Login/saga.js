...
import { USER_LOGIN, USER_RESET_PASSWORD, USER_SIGN_UP } from './actionTypes';
import store from 'reduxStore';
import { EMAIL_URL_MAP } from 'config/env/const';
import { RedirectUnAuthUser } from 'utils';

function identifyBiosLoginURLByEmail({ email }) {
...
function* userLogin({ payload }) {
  try {
    let url = null;
    const { email, password, history, sid, redirectTo } = payload;

    if (process.env.NODE_ENV === 'production') {
      url = identifyBiosLoginURLByEmail({ email });
    }
    const cred = {
      email,
      password,
...
async function signUp(payload) {
  return bios.initiateSignup(
    payload.email,
    payload?.sid,
    payload?.gcpMarketplaceToken,
  );
}

function* userSignUp({ payload }) {
  try {
    yield call(signUp.bind(this, payload));
...
async function resetPassword(payload) {
  return bios.initiateResetPassword(payload.email);
}

function* requestResetPassword({ payload }) {
  try {
    yield call(resetPassword.bind(this, payload));
    yield put(userRequestResetPasswordSuccess());
  } catch (e) {
    yield put(userRequestResetPasswordFailure());
  }
}
...
function* actionWatcher() {
  ...
  yield takeLatest(USER_RESET_PASSWORD, requestResetPassword);
}
