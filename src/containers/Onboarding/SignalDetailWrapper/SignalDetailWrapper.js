...
import SignalDetail from 'containers/SignalDetail';
import styles from '../NewSignalTeachBiosWrapper/styles';
...
const SignalDetailWrapper = ({
  ...
  // Set the signalDetail in onboardinge2e to current current signalDetail for the SignalDetail
  // component on loading this component. An initial import flow is also built and set here.
  useEffect(() => {
    setSignalDetail({});
    updateSignalDetail({ ...signalDetail, isNewEntry: true });
    ...
    if (importSourceId?.length > 0) {
      const subject = selectedEntity?.subjects.find((subj) =>
        subj.newName === '' ? subj.subjectName : subj.newName === activeEntity,
      );
            ...
            updatedSelectedEntity.subjects = updatedSelectedEntity.subjects.map(
              (subject) => {
                ...
                if (
                  ...
                  activeEntity !== signalDetailNew.signalName
                ) {
                  subject.newName = signalDetailNew.signalName;
                  subject.created = true;
                }
                return subject;
              },
...
const mapStateToProps = (state) => {
  ...
  const { signalDetail } = state?.onboardinge2e.global;
  const { signalDetail: signalDetailNew } = state?.signalDetail;
