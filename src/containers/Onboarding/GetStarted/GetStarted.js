...
import image from './guy_with_cape.gif';
import style from './style';

const { userClicks } = ipxl;

const GetStarted = ({ setStep, history }) => {
  return (
    <div className={css(style.wrapper)}>
      <div className={css(style.iconWrapper)}>
        <img src={image} className={css(style.iconSuperman)} alt="" />
      </div>
      <div className={css(style.text)}>
        bi(<span className={css(style.textHighlighted)}>OS</span>) delivers
        unparalleled speed to insights.
        <br /> Get going in 5 mins.
      </div>
      <Button
        type="primary"
        onClick={() => {
          ...
          userClicks({
            ...
            eventLabel: `Start Onboarding`,
            rightSection: 'None',
            mainSection: 'onboardingStart',
