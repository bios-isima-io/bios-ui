...
import MongoDBSourceProperties from 'containers/Integrations/components/IntegrationContents/ImportSources/Type/MongoDB/Tenant/SourceProperties';
import { integrationOnboardingActions } from './reducers';
import { cloneDeep } from 'lodash';

const { setMongodbOnboarding } = integrationOnboardingActions;

function MongoDBWrapper({
  mongodb,
  setMongodbOnboarding,
  errors = {},
  setErrors = () => {},
  disableFieldEdit,
}) {
  ...
  const setMongodbData = useCallback(
    (key, value) => {
      setMongodbOnboarding({ [key]: value });
    },
    [setMongodbOnboarding],
  );
  ...
  const setMongodbEndpointsData = (key, index, value) => {
    ...
    setMongodbData(key, endpointsCopy);
  };
  ...
  const setSSLMode = useCallback(
    (key, value) => {
      setMongodbOnboarding({ [key]: { mode: value } });
    },
    [setMongodbOnboarding],
  );

  return (
    <MongoDBSourceProperties
      errors={errors}
      setErrors={setErrors}
      srcUser={srcUser}
...
const mapStateToProps = (state) => {
  const { mongodb } = state?.onboardinge2e.integration;
...
MongoDBWrapper.propTypes = {
  setMongodbOnboarding: PropTypes.func,
  mongodb: PropTypes.object,
  disableFieldEdit: PropTypes.bool,
};

export default connect(mapStateToProps, mapDispatchToProps)(MongoDBWrapper);
