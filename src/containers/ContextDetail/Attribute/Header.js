...
const HeaderWrapper = ({
  ...
  const isSavedAttribute =
    !initialCreation &&
    contextDetailCopy?.attributes?.some(
      (attribute) => attribute._id === selectedAttribute._id,
    );
  ...
  return (
    <Header
      ...
      titleReadOnly={isSavedAttribute}
      title={selectedAttribute?.label}
      backLinkText={
        isMobile ? null : isEnriched ? 'Enriched Attribute' : 'Attribute'
      }
      ...
      actionPanel={() => {
        return (
          <AttributeHeader
            history={history}
            disableUpdateAction={isEnriched}
...
const mapStateToProps = (state) => {
  const { initialCreation } = state.contextDetail;
  return {
    initialCreation,
  };
};
