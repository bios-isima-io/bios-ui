...
const FeatureHeader = ({
  ...
  const saveFeature = () => {
    ...
    const currentFeatures = contextDetail?.features
      ? contextDetail.features
      : [];
  ...
  const deleteFeature = () => {
    ...
    const updatedFeatures = contextDetailCopy.features.filter((feature) => {
      return feature.featureName !== selectedFeature.featureName;
    });
...
const mapStateToProps = (state) => {
  const { validationErrors } = state.contextDetail;
  return {
    validationErrors,
  };
};
