...
import MultiShimSingleAttribute from './MultiShimSingleAttribute';
import shortid from 'shortid';

const MultiShimAttributePanel = ({
  selectedAttributes,
  ...
  const addNewAttribute = () => {
    const nextSelectedAttributes = [
      ...selectedAttributes,
      {
        alias: '',
        fillIn: '',
        source: selectedContexts.map((context) => {
          return {
            context: context,
            attribute: '',
  ...
  const updateAttribute = (attribute) => {
    const nextAttributes = selectedAttributes
      ?.map((attr) => (attr._id === attribute._id ? attribute : attr))
      .filter((attr) => !attr._deleted);
  ...
  return (
    <div className={css(enrichmentStyles.enrichmentPanelWrapper)}>
      <div
        className={css(enrichmentStyles.multiShimAddAttributeLabel)}
        onClick={addNewAttribute}
      >
        Click to Add Attribute
      </div>
      {selectedAttributes?.map((entity, index) => {
        return (
          <MultiShimSingleAttribute
            entity={entity}
            onUpdate={updateAttribute}
            index={index}
            lastIndex={index === selectedAttributes.length - 1}
...
MultiShimAttributePanel.propTypes = {
...
export default MultiShimAttributePanel;
