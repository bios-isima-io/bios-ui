...
const ContextPanel = ({
  selectedContexts,
  availableRemoteContexts,
  addContext,
  removeContext,
}) => {
  const filteredContexts = availableRemoteContexts?.filter((context) => {
    return !selectedContexts.includes(context.contextName);
  });

  const contextMenu = (
    <Menu
      className={css(commonStyles.menuWrapper)}
      onClick={({ key }) => addContext(key)}
    >
      ...
      <div className={css(styles.row)}>
        ...
        <Dropdown overlay={contextMenu} trigger={['click']}>
          <div className={css(styles.dropdownLabelWrapper)}>
            <div>Select context...</div>
            <i className="icon-chevron-down" />
          </div>
        </Dropdown>
      </div>

      <div className={css(enrichmentStyles.contextListWrapper)}>
        {selectedContexts.map((item) => {
          return (
            <div
              className={css(enrichmentStyles.contextListItemLabel)}
              key={item}
            >
              <div>{item}</div>
              <i
...
ContextPanel.propTypes = {
  ...
  availableRemoteContexts: PropTypes.instanceOf(Object),
  addContext: PropTypes.func,
  removeContext: PropTypes.func,
};
