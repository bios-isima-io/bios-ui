...
const ReportURLParams = ({
  ...
  const updateURLParams = () => {
    const data = {
      selectedSignalsList: selectedSignals?.map((sig) => sig.signalName),
      selectedMetrics,
    ...
    const urlStringData = JSON.stringify(data);
    const params = window.btoa(encodeURIComponent(urlStringData));
    history.replace({ pathname: location.pathname, search: 'data=' + params });
  };

  const buildURLParams = () => {
    if (
      (params?.reportId === '' || params?.reportId === undefined) &&
      signals &&
      signals.length > 0
    ) {
      updateURLParams();
...
const mapStateToProps = (state) => {
  ...
  const { graphData, cyclicalData, signalDataOrder, graphDataLoading, map } =
    state.report.data;
  ...
  const { allFilters, selectedFilters, filterOrder } = state.report.filters;
  return {
    signals,
    selectedSignals,
    selectedMetrics,
...
export default connect(mapStateToProps, null)(ReportURLParams);
