...
export function* fetchReportGraphDataSaga(action) {
  ...
  try {
    ...
    if (isMapChart) {
      ...
      let queryResult = [];
      if (cyclicalComparisonOn && statements?.length > 0) {
      ...
      resultData.forEach((result, index) => {
        ...
        if (index < queryResult.length) {
          statementData.push(queryResult[index]);
        }
      });
    } else if (groupByX === '' && groupByY !== '') {
      ({ signalDataOrder, statementData, queryStartTime } =
    ...
    if (calledFrom === 'report' || calledFrom === 'insights-overview-chart') {
      ...
      if (calledFrom === 'report' && !sideCompareReportId) {
        yield put(
      ...
      if (calledFrom === 'report' && sideCompareReportId) {
        yield put(
          reportGraphActions.setSideReportGraphData({
            graphData: statementData,
            signalDataOrder,
            cyclicalData,
            queryStartTime,
...
export function* fetchReportGraphDataSideCompareSaga(action) {
  yield call(fetchReportGraphDataSaga, action);
}
...
function* actionWatcher() {
  ...
  yield takeEvery(
    FETCH_REPORT_GRAPH_DATA_SIDE_COMPARE,
    fetchReportGraphDataSideCompareSaga,
  );
}
