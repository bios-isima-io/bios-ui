...
function* buildQueryGroupByX({
  ...
  for (const signal of selectedSignals) {
    ...
    if (onTheFly) {
      statementGraphData.onTheFly();
    }

    signalDataOrder.push(signalName);

    const queryResult = yield bios.multiExecute(statementGraphData.build());
    resultData.push(queryResult[0]);
    ...
    if (
      ...
      const whereClauseCyclical = cloneDeep(selectedFilters);
      if (!whereClauseCyclical[groupByX]) {
        whereClauseCyclical[groupByX] = [];
      }
      queryResult.dataWindows?.[0]?.records.forEach((record) => {
        if (record?.[0]) {
          if (whereClauseCyclical[groupByX].indexOf(record?.[0]) === -1) {
            whereClauseCyclical[groupByX].push(record?.[0]);
          }
        }
      });
      ...
      const filterWhereClauseCyclical = createWhereClauseWithFilters(
        whereClauseCyclical,
        {},
        attributeTypeMapping,
      );
      ...
      statements.push(
        bios
          ...
          .from(signalName)
          .groupBy(groupByX)
          .where(filterWhereClauseCyclical)
          .orderBy(...orderby)
          .limit(topX)
          .tumblingWindow(duration)
...
export default buildQueryGroupByX;
