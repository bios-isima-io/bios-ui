...
const buildDataForInsights = (reportInsights, metricsModifiers) => {
  ...
  const timezoneVal = timezone ?? moment.tz.guess();
  ...
  if (groupByX === '' || groupByX === undefined) {
    ...
    const chartTime = getDurationString(durationStart, duration, timezoneVal);
    ...
    const ccChartTime = getDurationString(ccStart, duration, timezoneVal);

    box1Hover = (
      <div>
        <div>{graphData?.series?.[1]?.name}</div>
        <div>{chartTime}</div>
      </div>
    );
    box1Measurement =
    ...
    box2Hover = (
      <div>
        <div>{graphData?.series?.[0]?.name}</div>
        <div>{ccChartTime}</div>
      </div>
    );
