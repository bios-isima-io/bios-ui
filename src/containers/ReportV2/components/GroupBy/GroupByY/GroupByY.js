...
import {
  groupByDataSketchDisabledMessage,
  GROUP_BY_Y_DISABLED_BUBBLE_CHART_MSG,
} from '../constant';
...
const { Option } = Select;
const { setGroupByY } = groupByActions;
...
const GroupByY = ({
  ...
  const setGroupByYAxisChange = (value) => {
    setGroupByY({ groupByY: value });
    if (value === '') {
      resetTop();
    }
    userClickEvent({
      history,
      eventLabel: `Groupby Y - select ${value}`,
      rightSection: 'None',
      mainSection: 'report',
      leftSection: 'insight',
    });
  ...
  if (isPackedBubble && groupByY !== '') {
    setGroupByYAxisChange('');
  }
  const updateDimensions = () => {
    ...
    if (groupByX && groupByX !== '') {
      dimensionList = dimensionList.filter((dim) => dim !== groupByX);
    }
  ...
  useEffect(() => {
    updateDimensions();
  }, [groupByX, selectedSignals, selectedMetrics, selectedFilters]);

  const checkIfGpyDisabled = () => {
    const hasPackedBubble = selectedMetrics.some(
      (sm) => sm.defaultGraphType === 'packedbubble',
    );
    if (
      checkGraphType({ selectedMetrics, graphType: 'funnel' }) ||
      checkGraphType({ selectedMetrics, graphType: 'map' })
    ) {
      return true;
    }
    if (hasPackedBubble && groupByX === '') {
  ...
  const getDisabledGroupByYMessage = () => {
    if (disableGroupBy) {
      return groupByDataSketchDisabledMessage('Y');
    }
    if (checkIfGpyDisabled()) {
      return GROUP_BY_Y_DISABLED_BUBBLE_CHART_MSG;
    }
    return null;
  };

  return (
    <div className="groupby-y-axis">
      <Tooltip title={getDisabledGroupByYMessage()}>
        {!isPackedBubble && (
          <Select
            disabled={disableGroupBy || checkIfGpyDisabled()}
            placeholder="Split-By Y"
            onChange={setGroupByYAxisChange}
            value={groupByY === '' ? 'Select' : groupByY}
            dropdownMatchSelectWidth={false}
            dropdownClassName="select-dropdown"
            className="select-container"
            suffixIcon={() => <i className="icon-chevron-down"></i>}
          >
            {groupByY !== '' && <Option value="" key=""></Option>}
...
export default connect(mapStateToProps, mapDispatchToProps)(GroupByY);
