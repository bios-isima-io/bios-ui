...
function ReportInfo({
  ...
  const details = (
    <div>
      <p>Report config summary</p>
      {selectedMetrics?.length > 0 && <b>Selected Metrics</b>}
      ...
      {windowSizeText && (
        <p>
          <b>Window Size:</b> {windowSizeText}
        </p>
      )}
      {ccEnabled && (
        <p>
          <b>Cyclical Comparison: </b>
          {getDurationString(ccStart, duration, timezoneText)}
        </p>
      )}
      {shouldAllowTopX && (
        <p>
      ...
      <p>
        <b>Timezone:</b> {timezoneText}
      </p>
      {filtersText && (
        <p>
          <b>Filters:</b> {filtersText}
        </p>
      )}
    </div>
  );
  return (
    <div>
      <Tooltip title={details}>
        <i
          className={`icon-Info report-info-icon ${css(iconStyles.IconInfo)}`}
        />
      </Tooltip>
    </div>
  );
