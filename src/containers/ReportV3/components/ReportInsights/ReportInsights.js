...
function ReportInsights({
  ...
  const {
    ...
    duration: {
      ...
      timeDuration: { duration, durationStart },
      timeZone: { timezone },
      windowSize: { windowSize },
    },
    groupBy: { groupByX },
    dataGranularity: { topX },
    filters: { selectedFilters },
  } = reportList?.[activeReportIndex];
  ...
  useEffect(() => {
    if (
      reportInsights?.[activeReportIndex] &&
      Object.keys(reportInsights?.[activeReportIndex])?.length > 0
    ) {
      setReportInsightsBoxes(
        assignIds(
          buildDataForInsights(
            reportInsights?.[activeReportIndex],
            metricsModifiers,
          ),
        ),
...
const mapStateToProps = (state) => {
  ...
  const { reportInsights } = state?.reportV3.reportInsights;
