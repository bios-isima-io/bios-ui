...
const getDimensionList = ({
  ...
  if (selectedContexts?.length === 0 || selectedMetrics?.length === 0) {
    return [];
  }

  const contextMap = {};
  selectedContexts?.forEach((context) => {
    contextMap[context?.contextName] = context;
  });

  const contextsMetricsMapping = getEntitiesMetricsMapping(
    selectedMetrics,
    selectedContexts,
    type,
  );
  ...
  const dimensionsList = Object.keys(contextsMetricsMapping).reduce(
    (contextAcc, context) => {
      let metricsList = new Set();
      metricsList.add(`count()`);
      contextMap[context]?.availableAttributes?.forEach((attribute) => {
        const metrics = [
      ...
      const MetricsWithThatContext = contextsMetricsMapping[context];
      contextMap?.[context]?.features?.forEach((feature) => {
        if (containsAll(allMetricsInFeature, MetricsWithThatContext)) {
          if (containsAll(feature?.dimensions, activeDimensions)) {
            const dimensions = [...(feature?.dimensions ?? [])];
            contextAcc.push(dimensions);
          }
        }
      }, []);
      return contextAcc;
  ...
  if (dimensionsList && dimensionsList.length > 0) {
    return getIntersectionForArrays(dimensionsList);
  } else {
    return [];
  }
};

const getAttributeFromMetrics = (ContextsMetricsMapping) => {
  let uniqueMetrics = new Set();
  Object.keys(ContextsMetricsMapping).forEach((context) => {
    ContextsMetricsMapping?.[context].forEach((metric) => {
...
const getEnabledDimensionsGroupBy = (selectedContexts, activeDimensions) => {
  let enabledDimensionFilter = new Set();
  selectedContexts.forEach((context, context_index) => {
    context?.[context_index]?.postStorageStage?.features.forEach((feature) => {
      if (
        activeDimensions.length === 0 ||
        containsAll(feature.attributes, activeDimensions)
      ) {
        feature?.dimensions?.forEach((item) =>
          enabledDimensionFilter.add(item),
        );
      }
    });
  });
  return [...enabledDimensionFilter];
};
...
const getDimensionListGroupBy = ({
  ...
  const contextMap = {};
  selectedContexts.forEach((context) => {
    contextMap[context?.[entityName]] = context;
  });
  ...
  const attributes = getAttributeFromMetrics(ContextMetricsMapping);

  const dimensionsList = Object.keys(ContextMetricsMapping).reduce(
    (contextAcc, context) => {
      const contextDimensions = getEnabledDimensionsGroupBy(
        [
          selectedContexts.filter(
            (contextItem) => contextItem.contextName === context,
          ),
        ],
        attributes,
      );
      if (contextDimensions && contextDimensions.length > 0) {
        contextAcc.push([...contextDimensions]);
      } else {
        contextAcc.push([]);
      }
      return contextAcc;
...
export { getDimensionList, getDimensionListGroupBy };
