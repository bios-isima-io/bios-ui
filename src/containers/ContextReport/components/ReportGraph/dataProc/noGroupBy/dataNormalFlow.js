...
const evaluateDerivedMetrics = ({
  ...
  terms.forEach((term) => {
    const value = fetchValue(groupByXValue, term) ?? 0;

    if (value && newDataTotal?.[term]) {
      newDataTotal[term] = newDataTotal[term] - value;
    }
...
export const buildDataSetNoGroupByNormalFlow = ({
  contextDataOrder,
  graphData,
  groupByX,
  chartType,
