...
const getContextMetricAtt = (measurement) => {
  try {
    const [contextName, val] = measurement?.split('.');
    const [metric, attribute] = val?.slice(0, -1)?.split('(');

    return { contextName, metric, attribute };
  } catch {
    return { contextName: '', metric: '', attribute: '' };
  }
};
...
const buildTagValue = ({
  ...
  const context = selectedContexts.filter(
    (con) => con.contextName === contextName,
  );
  let unitDisplayName = '';
  let unitDisplayPosition = '';
  if (context && context.length === 1) {
    const att = context?.[0]?.attributes?.find(
...
const getSimpleMetricModifier = (selectedContexts, metricItem, modifiers) => {
  const { contextName, metric, attribute } = getContextMetricAtt(metricItem);
...
const getDerivedMetricModifier = (selectedContexts, metricItem, modifiers) => {
  let measurements = measurementToArray(
    selectedContexts,
    metricItem,
    'context',
  );
  ...
  const metricTagMap = measurements?.reduce((acc, met) => {
    acc[met] = getSimpleMetricModifier(selectedContexts, met, modifiers);
    return acc;
  }, {});
...
export { getSimpleMetricModifier, getDerivedMetricModifier };
