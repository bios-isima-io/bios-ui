...
import { rightPanelActions } from '../ReportSettingsToggle/reducers';
import CollapsableWrapper from 'containers/components/CollapsableWrapper';
import Metrics from './MetricsNew';
  ...
  return (
    <div className="advance-settings">
      <div className="header">
        ...
        <div className="advance-setting-close" onClick={() => closeDrawer()}>
          <i className="icon icon-close" />
        </div>
      </div>
      <CollapsableWrapper
        activePanel={activeTabRightPanel}
        panels={panels}
        setActiveRightPanel={setActiveTabRightPanel}
      />
    </div>
  );
}
...
const mapStateToProps = (state) => {
  const { activeTabRightPanel } = state.contextReport.rightPanel;
  return {
    activeTabRightPanel,
  };
};
