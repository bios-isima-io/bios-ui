...
function Card({
  ...
  const showReport = () => {
    if (
      card?.cardType !== 'invalidCardNoContext' &&
  ...
  return (
    <div>
      ...
      <Draggable
        ...
        index={cardIndex}
        isDragDisabled={
          disabledDragDrop || card.cardType === 'invalidCardNoContext'
        }
      >
        {(provided) => (
          <div
          ...
          >
            ...
            <div className="card-indicator">{count?.toLocaleString()}</div>
...
const mapStateToProps = (state) => {
  ...
  const { insightsData, gridData } = state?.insights?.insightsContextGrid;
