...
import { PageTitle, SortControl } from 'containers/components';
import { useDeviceDetect } from 'common/hooks';
import { SORT_CONFIG, USER_HEADER, USER_HEADER_MOBILE } from './constant';
    ...
    return (
      <>
        ...
        {filterUserList.length > 0 ? (
          <Listing
            noItemMsg="No user found"
            header={window.innerWidth > 1200 ? USER_HEADER : USER_HEADER_MOBILE}
            data={filterUserList}
            itemClick={({ item }) => {
              setSelectedUser(item);
              setShowRightPanel(true);
              setRightPanelType('view_user_details');
              userClicks({
                pageURL: history?.location?.pathname,
                pageTitle: document.title,
                pageDomain: window?.location?.origin,
                eventLabel: `Open User Detail`,
                rightSection: 'None',
                mainSection: 'users',
                leftSection: 'user',
              });
            ...
            selectedUserId={
              selectedUser && selectedUser.userId ? selectedUser.userId : null
            }
          />
        ) : (
          <EmptyPlaceholder
  ...
  return (
    <div className={css(commonStyles.pageContentWrapper)}>
      <div className={css(commonStyles.pageContent)}>
        <TitleHelmet title="Users" />
        <PageTitle label="Users" actions={headerActions} />
